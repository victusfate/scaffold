...
|---|---|---|---|
| loop | `/(?:^\/loop\b)/i` | `skills/loop.md` | Schedule recurring work, inspect or stop loops, and route user steering to an active main-orchestrator loop by default; portable macOS/Linux/WSL/Windows driver |
...
| jev-typesafe | `/(?:^\/jev-typesafe\b)|(?:jev|typesafe|system\s*1|jev\s*evaluate|openrouter\s+jev)/i` | `skills/jev-typesafe.md` | Add Jev (System 1) judgments to pi via OpenRouter: install pi-typesafe, drop the jev-openrouter.ts extension, and route micro-decisions to typesafe/jev-1.13 through jev_evaluate |
