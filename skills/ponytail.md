...
review-and-fix pass over a diff).
...
- Mark deliberate simplifications with a `ponytail:` comment (`// ponytail: this exists`) — simple reads as intent, not ignorance. A shortcut with a known ceiling (global lock, O(n²) scan, naive heuristic)? The comment names the ceiling and upgrade path: `# ponytail: global lock, per-account locks if throughput matters`.
...
mode": revert. Level persists until changed or session end. The shortest path to
done is the right path.
